...
def check_and_install_dependencies():
    # DÜZELTME 1: Eğer uygulama .exe olarak (frozen) çalışıyorsa pip işlemini atla!
...
class AppicTestStudio(ctk.CTk):
    def __init__(self):
        ...
        # DÜZELTME 2: Gömülü adb.exe dosyasını geçici bellekten (sys._MEIPASS) okuma mantığı
        if hasattr(sys, '_MEIPASS'):
            self.adb_path = os.path.join(sys._MEIPASS, "adb.exe")
        else:
            self.adb_path = os.path.join(self.main_dir, "adb.exe")
